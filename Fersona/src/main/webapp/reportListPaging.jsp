...
<body>
	...
	<div class="contain">
		...
		<!-- 제보/신고 리스트 -->
		<p id="totalLeft">총 !몰라!건이 검색되었습니다.</p>
		<table class="table">
...
function list_write(data_list){
	...
	for(var i =0 ; i<data_list.length;i++){//jquery 문을통해 태그안의 내용을 바꾸어준다 --> .text()사용
		...
		$('#rep_no_'+i).text(data_list[i].rep_no);
		$('#rep_cate_'+i).text(data_list[i].rep_cate);
		$('#rep_con_'+i).text(data_list[i].rep_con);
		$('#rep_date_'+i).text(data_list[i].rep_date);
		$('#rep_wri_'+i).text(data_list[i].rep_wri);
